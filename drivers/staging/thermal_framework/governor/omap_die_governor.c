...
static int omap_safe_zone(struct list_head *cooling_list, int cpu_temp)
{
	...
	list_for_each_entry(cooling_dev, cooling_list, node) {
		if (cooling_dev->dev_ops &&
			...
			list_add(&cooling_dev->node, &cooling_ops);
			goto out;
		} else {
...
out:
	if (list_empty(&cooling_ops)) {
		pr_err("%s: No Cooling devices registered\n",
			__func__);
		return -ENODEV;
	} else {
		thermal_cooling_set_level(&cooling_ops, 0);
		list_del_init(&cooling_ops);
	}
...
{
	pr_info("%s:FATAL ZONE (hot spot temp: %i)\n", __func__, cpu_temp);
...
{
	int cpu_temp = 0;
	/* TODO: May need these pulled from a configurable
	 * source. For now just hardcode */
	int panic = 110000;
	int alert = 100000;
	int monitor = 85000;
	...
	if (cpu_temp >= OMAP_CPU_THRESHOLD_FATAL) {
		...
		return FATAL_ZONE;
	} else if (cpu_temp >= panic) {
		/* TODO: make the required level a configurable property */
		omap_panic_zone(cooling_list, cpu_temp);
		return PANIC_ZONE;
	} else if (cpu_temp < (panic - HYSTERESIS_VALUE)) {
		if (cpu_temp >= alert) {
			/* TODO: make the required lvl configurable */
			omap_alert_zone(cooling_list, cpu_temp);
			return ALERT_ZONE;
		} else if (cpu_temp < (alert - HYSTERESIS_VALUE)) {
			if (cpu_temp >= monitor) {
				omap_monitor_zone(cooling_list, cpu_temp);
				return MONITOR_ZONE;
			} else if (cpu_temp < (monitor - HYSTERESIS_VALUE)) {
...
static int __init omap_die_governor_init(void)
{
	...
	if (omap_gov) {
		...
		pr_info("%s:Cannot allocate memory\n", __func__);
		return -ENOMEM;
	}
...
MODULE_DESCRIPTION("Thermal class driver");
MODULE_LICENSE("GPL");

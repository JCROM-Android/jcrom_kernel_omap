...
static struct thermal_domain *therm_dom;
 ...
 *
 * Returns 0 for a succesfull call to the cooling agent. ENODEV if a cooling
 ...
 *
 * Returns 0 for a succesfull call to a governor. ENODEV if a governor does not
 * exist.
 */
	...
	list_for_each_entry(thermal_domain, &thermal_domain_list, node)
		if (!strcmp(thermal_domain->domain_name, tdev->domain_name)) {
			...
			if (!thermal_domain->cooling_agents.next) {
				pr_err("%s:No cooling agents\n", __func__);
				goto out;
			}

			if (thermal_domain->governor->dev_ops &&
...
report:
	if ((thermal_domain->temp_sensor->dev_ops) &&
			...
			pr_err("%s:Getting temp is not supported\n",
				__func__);
			ret = -EOPNOTSUPP;
		}
out:
...
check_domain:
	if ((domain->temp_sensor) &&
		...
		pr_info("%s:Not all components registered for %s domain\n",
 ...
 *
 * Returns 0 for a succesful registration of the governor to a domain
...
out:
	mutex_unlock(&thermal_domain_list_lock);
	ret = thermal_init_thermal_state(tdev);
...
int thermal_sensor_dev_register(struct thermal_dev *tdev)
{
	...
	if ((list_empty(&thermal_domain_list)) ||
	    (!therm_dom)) {
		pr_debug("%s:Need to init the domain\n", __func__);
		goto init_sensor;
	} else {
		mutex_lock(&thermal_domain_list_lock);
		list_for_each_entry(domain, &thermal_domain_list, node) {

...
static irqreturn_t omap_talert_irq_handler(int irq, void *data)
{
	...
	/* Need to protect against spasmatic temp sensors. If the temp sensor
	   throws an invalid value then we should not try to report the data */
...
static int __devinit omap_temp_sensor_probe(struct platform_device *pdev)
{
	...
	/*if (__raw_readl(OMAP4_CTRL_MODULE_CORE +

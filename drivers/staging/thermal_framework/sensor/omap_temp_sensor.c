					...
					int min, int max)
{
	...
	if (((new_hot < OMAP_ADC_START_VALUE ||
	      new_hot > OMAP_ADC_END_VALUE)) ||
	   ((new_cold < OMAP_ADC_START_VALUE ||
		new_cold > OMAP_ADC_END_VALUE))) {
...
static int omap_set_temp_thresh(struct thermal_dev *tdev, int min, int max)
{
	...
	return omap_set_thresholds(temp_sensor, min, max);
}
	...
	if (enable) {
		...
		while (!(temp | OMAP4_CLEAN_STOP_MASK))
			temp = omap_temp_sensor_readl(temp_sensor,
							BGAP_STATUS_OFFSET);
		/* Gate the clock */
		pm_runtime_put_sync(&temp_sensor->pdev->dev);
		if (ret) {

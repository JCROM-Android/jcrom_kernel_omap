...
static int wl127x_rfkill_probe(struct platform_device *pdev)
{
	...
	if (pdata->bt_nshutdown_gpio != -1) {
		rc = gpio_request(pdata->bt_nshutdown_gpio, "wl127x_bt_nshutdown_gpio");
		if (unlikely(rc))
			return rc;
	} else {
		return 0;
		/* FM cannot exist without BT on wl127x as of yet,
		 * so bt_nshutdown_gpio=-1 and fm_enable_gpio!=-1 isn't valid
		 */
	}
	...
	wl127x_rfkill_set_power((void*)pdata->bt_nshutdown_gpio, default_state);

	pdata->rfkill[WL127X_BLUETOOTH] = rfkill_allocate(&pdev->dev, RFKILL_TYPE_BLUETOOTH);
	...
	pdata->rfkill[WL127X_BLUETOOTH]->data = (void *) pdata->bt_nshutdown_gpio;
	pdata->rfkill[WL127X_BLUETOOTH]->toggle_radio = wl127x_rfkill_set_power;
	...
	/* registering FM rfkill device */
	if (pdata->fm_enable_gpio != -1) {
	        rc = gpio_request(pdata->fm_enable_gpio, "wl127x_fm_enable_gpio");
		if (unlikely(rc))
			return rc;
	} else {
		return 0;
	...
	wl127x_rfkill_set_power((void*)pdata->fm_enable_gpio, default_state);

	pdata->rfkill[WL127X_FM] = rfkill_allocate(&pdev->dev, RFKILL_TYPE_FM);
	...
	pdata->rfkill[WL127X_FM]->toggle_radio = wl127x_rfkill_set_power;
...
static int wl127x_rfkill_remove(struct platform_device *pdev)
{
	...
	if (pdata->rfkill[WL127X_BLUETOOTH]) {

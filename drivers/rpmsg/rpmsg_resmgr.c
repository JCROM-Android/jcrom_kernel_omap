...
static int rprm_auxclk_request(void **rh, struct rprm_auxclk *obj)
{
	...
	if ((obj->clk_id < AUX_CLK_MIN) || (obj->clk_id > AUX_CLK_MAX)) {
	...
	sprintf(clk_name, "auxclk%d_ck", obj->clk_id);
	...
	sprintf(src_clk_name, "auxclk%d_src_ck", obj->clk_id);
...
static int rprm_regulator_request(void **rh, struct rprm_regulator *obj)
{
	...
	if (obj->regulator_id > REGULATOR_MAX) {
	...
	reg_name = regulator_name[obj->regulator_id - 1];
	rd->reg_p = regulator_get(NULL, reg_name);
	if (!rd->reg_p) {
...
static int rprm_gpio_request(void **rh, struct rprm_gpio *obj)
{
	...
	ret = gpio_request(obj->gpio_id , "rpmsg_resmgr");
	if (ret) {
...
static int rprm_sdma_request(void **rh, struct rprm_sdma *obj)
{
	...
	if (obj->num_ch > MAX_NUM_SDMA_CHANNELS) {
		pr_err("Not able to provide %u channels\n", obj->num_ch);
		return -EINVAL;
	}

	for (i = 0; i < obj->num_ch; i++) {
...
static int _printf_auxclk_args(char *buf, struct rprm_auxclk *obj)
{
	return sprintf(buf,
		...
		"ParentSrc:%d\n"
		"ParentSrcRate:%d\n",
		obj->clk_id, obj->clk_rate,
		obj->parent_src_clk, obj->parent_src_clk_rate);
}
...
static int _printf_regulator_args(char *buf, struct rprm_regulator *obj)
{
	return sprintf(buf,
		...
		"min_uV:%d\n"
		"max_uV:%d\n",
		obj->regulator_id, obj->min_uV, obj->max_uV);
}
...
static int _print_res_args(char *buf, struct rprm_elem *e)
{
	switch (e->type) {
	case RPRM_GPTIMER:
		return _printf_gptimer_args(buf, (void *)e->res);
	case RPRM_AUXCLK:
		return _printf_auxclk_args(buf, (void *)e->res);
	case RPRM_REGULATOR:
		return _printf_regulator_args(buf, (void *)e->res);
	case RPRM_GPIO:
		return _printf_gpio_args(buf, (void *)e->res);
	case RPRM_SDMA:
		return _printf_sdma_args(buf, (void *)e->res);
	}
	return 0;
}

...
static int rprm_resource_alloc(struct rprm *rprm, u32 src, int *res_id,
				int type, void *data)
{
	void *handle = NULL;
	struct rprm_elem *e;
	...
	spin_lock(&rprm->lock);
	ret = idr_get_new_above(&rprm->list, e, 1, res_id);
...
static int rprm_resource_free(struct rprm *rprm, u32 src, int res_id)
{
	int ret = 0;
	struct rprm_elem *e;

	spin_lock(&rprm->lock);
	e = idr_find(&rprm->list, res_id);
	...
	if (!ret)
		idr_remove(&rprm->list, res_id);
	spin_unlock(&rprm->lock);
out:
	return ret;
}

static int rprm_idr_free_if_src(int id, void *p, void *data)
{
...
static int rprm_idr_free_all_by_src(struct rprm *rprm, u32 src)
{
	int id;
	...
	spin_lock(&rprm->lock);
	for (;;) {
		id = idr_for_each(&rprm->list, rprm_idr_free_if_src,
				(void *)src);
		if (!id)
			break;
		idr_remove(&rprm->list, id);
	}
	spin_unlock(&rprm->lock);
			 ...
			 int acq, int size, void *data)
{
	...
	case RPRM_REQ_EXIT:
		return rprm_idr_free_all_by_src(rprm, src);
	}
...
static void __devexit rprm_remove(struct rpmsg_channel *rpdev)
{
	...
	idr_for_each(&rprm->list, rprm_idr_free_res, NULL);

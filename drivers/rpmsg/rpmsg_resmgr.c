...
static int rpres_resource(int type, bool req, void *data)
{
	int err;

	/* if request check if the resource is in used */
	if (req && _find_res(type, data))
		return -EBUSY;

	/* if free check if the resource exists */
	if (!req && !_find_res(type, data))
		return -ENOENT;

	switch (type) {
	case RPRES_GPTIMER:
		err = rpres_gptimer(data, req);
		break;
	case RPRES_IVAHD:
	case RPRES_IVASEQ0:
	case RPRES_IVASEQ1:
		err = rpres_iva(type, data, req);
		break;
	case RPRES_L3BUS:
		err = rpres_l3_bus(data, req);
		break;
	case RPRES_ISS:
		err = rpres_iss(data, req);
		break;
	default:
		err = -ENOENT;
	}
		...
		void *priv, u32 src)
{
	...
	pr_err("proc %d\n"
		...
		"private  %p\n"
		"data size %d\n",
		obj->proc, obj->res_type, obj->request, obj->priv,
		obj->data_sz);

	err = rpres_resource(obj->res_type, !!obj->request, obj->data);
	if (err)

...
static struct rprm_cstrs_data def_data = {
	.frequency	= 0,
	.bandwidth	= -1,
	.latency	= -1,
};
...
static int _set_cstrs(struct rprm_elem *e, struct rprm_cstrs_data *c)
{
	int ret;
	u32 mask = 0;

	if (c->mask & RPRM_SCALE) {
		ret = rpres_set_cstrs(e->handle, RPRES_SCALE, c->frequency);
		if (ret)
			goto err;
		mask |= RPRM_SCALE;
		e->cstrs->frequency = c->frequency;
	}

	if (c->mask & RPRM_LAT) {
		ret = rpres_set_cstrs(e->handle, RPRES_LAT, c->latency);
		if (ret)
			goto err;
		mask |= RPRM_LAT;
		e->cstrs->latency = c->latency;
	}

	if (c->mask & RPRM_BW) {
		ret = rpres_set_cstrs(e->handle, RPRES_BW, c->bandwidth);
		if (ret)
			goto err;
		mask |= RPRM_BW;
		e->cstrs->bandwidth = c->bandwidth;
	}
err:
	c->mask = mask;
	return ret;
...
static int rprm_set_cstrs(struct rprm *rprm, u32 addr, int res_id,
			   void *data, bool set)
{
	int ret = 0;
	struct rprm_elem *e;
	...
	if (set) {
		ret = _set_cstrs(e, data);
		if (!ret) {
			e->cstrs->mask |=
					((struct rprm_cstrs_data *)data)->mask;
			goto out;
		}
	}
	def_data.mask = ((struct rprm_cstrs_data *)data)->mask;
	if (def_data.mask)
		_set_cstrs(e, &def_data);
out:
...
static int rprm_rpres_request(struct rprm_elem *e, int type)
{
	...
	e->cstrs = kzalloc(sizeof(*(e->cstrs)), GFP_KERNEL);
	if (!(e->cstrs))
		return -ENOMEM;
...
{
	if (e->cstrs && e->cstrs->mask) {
		def_data.mask = e->cstrs->mask;
		_set_cstrs(e, &def_data);
	}
	kfree(e->cstrs);
			...
			void *priv, u32 src)
{
	...
	case RPRM_REQ_CSTRS:
		r_sz = len - sizeof(*req);
		if (r_sz != sizeof(struct rprm_cstrs_data)) {
			r_sz = 0;
			ret = -EINVAL;
			break;
		}
		ret = rprm_set_cstrs(rprm, src, req->res_id, req->data, true);
		if (ret)
			dev_err(dev, "set constraints failed! ret %d\n", ret);
		break;
	case RPRM_REL_CSTRS:
		ret = rprm_set_cstrs(rprm, src, req->res_id, req->data, false);
		if (ret)
...
static int _printf_constraints_args(char *buf, struct rprm_elem *e)
{
	return sprintf(buf,
		...
		"Latency:%ld\n"
		"Bandwidth:%ld\n",
		e->cstrs->mask, e->cstrs->frequency,
		e->cstrs->latency, e->cstrs->bandwidth);
}

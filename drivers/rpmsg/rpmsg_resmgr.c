...
static int _set_constraints(struct rprm_elem *e, struct rprm_constraints_data *c)
{
	...
	if (c->mask & RPRM_SCALE) {
		ret = rpres_set_constraints(e->handle, RPRES_CONSTRAINT_SCALE, c->frequency);
		if (ret)
			goto err;
		mask |= RPRM_SCALE;
	...
	if (c->mask & RPRM_LAT) {
		ret = rpres_set_constraints(e->handle, RPRES_CONSTRAINT_LATENCY, c->latency);
		if (ret)
			goto err;
		mask |= RPRM_LAT;
	...
	if (c->mask & RPRM_BW) {
		ret = rpres_set_constraints(e->handle, RPRES_CONSTRAINT_BANDWIDTH, c->bandwidth);
		if (ret)
			goto err;
		mask |= RPRM_BW;

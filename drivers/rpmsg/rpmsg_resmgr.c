...
static int _set_constraints(struct rprm_elem *e, struct rprm_constraints_data *c)
{
	int ret;
	u32 mask = 0;

	if (c->mask & RPRM_SCALE) {
		ret = rpres_set_constraints(e->handle, RPRES_SCALE, c->frequency);
		if (ret)
			goto err;
		mask |= RPRM_SCALE;
	...
	if (c->mask & RPRM_LAT) {
		ret = rpres_set_constraints(e->handle, RPRES_LAT, c->latency);
		if (ret)
			goto err;
		mask |= RPRM_LAT;
	...
	if (c->mask & RPRM_BW) {
		ret = rpres_set_constraints(e->handle, RPRES_BW, c->bandwidth);
		if (ret)
			goto err;
		mask |= RPRM_BW;
			   ...
			   void *data, bool set)
{
	...
	if (e->constraints->mask == NO_CSTRS) {
...
#ifdef RPRM_DEBUG_CONSTRAINTS
	if (!strcmp(res_name, "rpres_iva")) {
		_set_constraints(e, &test_data);
		e->cstrs->mask = test_data.mask;
	}
#endif
	return 0;
}
			...
			void *priv, u32 src)
{
	...
	case RPRM_DISCONNECT:
		...
		return;
	case RPRM_REQ_CSTRS:
		ret = rprm_set_constraints(rprm, src, req->res_id, req->data, true);
		if (ret)
			dev_err(dev, "set constraints failed! ret %d\n", ret);
		break;
	case RPRM_REL_CSTRS:
		ret = rprm_set_constraints(rprm, src, req->res_id, req->data, false);
		if (ret)

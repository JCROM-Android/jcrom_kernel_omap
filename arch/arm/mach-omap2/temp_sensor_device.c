...
static int temp_sensor_dev_init(struct omap_hwmod *oh, void *user)
{
	...
	if (!temp_sensor_pdata) {
		pr_err
		    ("%s: Unable to allocate memory for %s temp_sensor_data.Error!\n",
			__func__, oh->name);
		return -ENOMEM;
	}
	if (i == 0) {
		temp_sensor_pdata->offset =
		    OMAP4_CTRL_MODULE_CORE_TEMP_SENSOR;
	}
...
int __init omap_devinit_temp_sensor(void)
{
	...
	return omap_hwmod_for_each_by_class("ctrl_module", temp_sensor_dev_init,
									NULL);
}
